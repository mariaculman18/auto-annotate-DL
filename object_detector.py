########################################################################################################
# Code sample: How to easily detect objects in images without training a target object detection model?

# Deep Learning (DL) is an area of Machine Learning where algorithms are modeled to imitate human rationale. Among the most important application areas of DL are computer vision and natural language processing, impacting our everyday life when we access our phone through facial recognition authorization, interact with a company's chatbot, or get support from a virtual assistant like Alexa.
...
# Thus, the code sample focuses on enabling an object detection application that does not require model training by relying on the latest advances in self-supervised learning (DINO), transformers (ViT), and convolutionals (ConvNeXt). This code is based on my Ph.D. work, where I propose data generation and model training strategies to overcome the limitation of insufficient data in non-conventional computer vision applications for precision agroforestry and agriculture. The provided detection results are noisy, as objects are not perfectly located or classified. Still, they can be used as a baseline to generate a dataset for a target application automatically. This is especially advantageous since the manual generation of a dataset is labor-intensive and time-consuming.

# In short:
# * What? Detecting objects in images without model training.
...
# * Output: images with boxes containing objects. Each detection displays the most probable class of the object and the confidence (0-1) the model gives to the inference.

# How does it work?

# Following the flowchart (see Flowchart.png):
...
# 4. Resulting object detections (discoveries + classes) are displayed in the original image accompanied by the confidence score.
...
# 2. Run the code. If running from the Python version, see in the terminal the name of the image and the detection results of classes and confidences. And if running from the Jupyter Notebook version see these data besides the images with objects located, classified, and the attributed confidence.
# 3. Check the output folder to see the resulting images.

# Notes:
# * Modify the potential number and extent of the detected objects by changing the parameters: number_seeds and correlation_th.
# * Image examples from my personal files and [ImageNet](https://www.image-net.org/), a generic dataset, have been placed for demonstration purposes. These are fruits but the code sample is extensible to any image.
